...
<template>
    <div class="px-4 pt-12 flex flex-col items-start w-full h-screen bg-black bg-opacity-5">
        ...
        <div>
            ...
            <h2 class="text-3xl">{{ time.year }} - {{ time.getMonth() }}</h2>
        </div>
        ...
        <div class="flex flex-col items-start h-1/4 overflow-y-scroll overflow-x-hidden w-full pt-2">
            <SelectBtn @select="ff" v-for="y in years" :key="y.value" :data="y" :name="'year'"/>
        </div>

        <!-- Month picker -->
        <h2 class="text-xl">Month</h2>
        <hr class="bg-white border-none bg-opacity-20 h-[1px] w-full">
        <div class="flex flex-col items-start h-3/4 overflow-y-scroll overflow-x-hidden w-full pt-2">
            <SelectBtn @select="f" v-for="m in validMonths" :key="m.value" :data="m" :name="'month'"/>
        </div>
